...
const schemaContent = `import { FormGroup, FormControl, Validators } from '@angular/forms';
...
const serviceContent = `import { Injectable, Injector } from '@angular/core';
...
const listPageTsContent = `import { Component } from '@angular/core';
...
const listPageHtmlContent = `<ion-header>
...
const listComponentTs = `import { Component, Injector, OnInit, OnDestroy } from '@angular/core';
...
@Component ({
  ...
  styleUrls: ['{{entityPlural}}-list.component.scss', '../../../core/abstract/components/abstract-list.component.scss'],
})
export class {{EntityPlural}}ListComponent extends AbstractListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    super(injector, {{EntityPlural}}Schema, {{EntityPlural}}Service);
    if (this.debug) { console.log(this.constructor.name + '.constructor()'); }
  }
...
const listComponentHtml = `<ion-header>

<ion-toolbar *ngIf="isSelectRowMode">
  <ion-buttons slot="start">
  ...
  <ion-grid fixed>
    <ion-row>
      <ion-col size="12" size-sm="12" size-md="6" size-xl="4" *ngFor="let row of rows | filter:search:list.filter | orderBy:list.orderBy">
        ...
        <ion-item-sliding>
          <ion-item (click)="select(row)" [detail]="!isSelectRowMode" [ngClass]="{ selected: isSelected(row)}" class="item-color">
...
const detailPageTs = `import { Component, Injector, OnInit, OnDestroy } from '@angular/core';
...
const detailPageHtml = `<ion-header>
